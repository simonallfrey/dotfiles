...
return {
  ...
  opts = {
    autocmds = {
      -- 1. DISABLE the default 'checktime' autocommand from your screenshot
      checktime = false,

      -- 2. CREATE a new safe version that checks for the command window
      checktime_safe = {
        {
          event = { "FocusGained", "TermClose", "TermLeave" },
          desc = "Check if buffers changed on editor focus (Safe)",
          callback = function()
            -- ONLY run checktime if we are NOT in the command-line window (q:)
